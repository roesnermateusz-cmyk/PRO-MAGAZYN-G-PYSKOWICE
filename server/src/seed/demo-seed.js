 ...
 * walidację co ręczne wprowadzanie — generator jest jednocześnie testem dymnym
 * całej ścieżki księgowania.
 *
 * Moduł jest wspólny dla wersji sieciowej i jednoplikowej: skrypt
 * `server/scripts/seed.mjs` wczytuje plik JSON z dysku i woła `seedDatabase`,
 * a wersja jednoplikowa podaje ten sam JSON wklejony przez generator pakietu.
 * Dzięki temu obie wersje pokazują dokładnie ten sam komplet dokumentów.
 *
 * Generator liczb losowych jest deterministyczny: ten sam plik wejściowy daje
 * zawsze ten sam zestaw danych. To celowe — zmiana liczby dokumentów bez
 * zmiany pliku wejściowego oznacza regresję w silniku, nie inny los.
 */
import db from '../db/index.js';
import { createUser } from '../modules/users/users.service.js';
import {
  warehouses, partners, vehicles, forest, loadingPlaces, products,
} from '../modules/catalog/catalog.service.js';
import { createOperation } from '../modules/operations/operations.service.js';
import { createChain } from '../modules/operations/chain.service.js';

/** Ziarno generatora — stałe, żeby wynik był powtarzalny. */
const SEED_STATE = 20260901;

const SIGNATURES = ['Piotr Nowak', 'Anna Kowalczyk', 'Mateusz Roesner'];

/** Ile dni wstecz sięga generowana historia. */
const DAYS_BACK = 60;

/**
 * @param {object} seed zawartość `server/seed/demo-data.json`
 * @param {object} options
 * @param {boolean} [options.reset] czyści rejestr dokumentów przed generowaniem
 * @param {object} options.ctx kontekst użytkownika (autor dokumentów)
 * @param {(msg:string)=>void} [options.log] podgląd postępu
 * @returns {{chains:number, documents:number, purchases:number, sales:number,
 *            users:Array, summary:object, stock:Array, warnings:string[]}}
 */
export function seedDatabase(seed, { reset = false, ctx, log = () => {} } = {}) {
  let rngState = SEED_STATE;
  const rnd = () => {
    rngState = (rngState * 1103515245 + 12345) & 0x7fffffff;
    return rngState / 0x7fffffff;
  };
  const between = (min, max, decimals = 0) => Number((min + rnd() * (max - min)).toFixed(decimals));
  ...
  if (reset) {
    log('Czyszczenie rejestru dokumentów…');
    db.tx(() => {
  ...
  log(`Generowanie dokumentów za ostatnie ${DAYS_BACK} dni…`);
  ...
  const stock = db.all(
    'SELECT product_name, ROUND(qty_mp, 2) AS mp FROM v_stock_current WHERE ABS(qty_mp) > 0.01 ORDER BY qty_mp DESC',
  );

  return { ...stats, users: createdUsers, summary, stock, warnings };
}
