...
/**
 * Kopia zapasową wykonywana natywnym mechanizmem SQLite (VACUUM INTO),
 * który tworzy spójny obraz bazy bez przerywania pracy użytkowników.
 */
export async function createBackup(db: Db, kind: 'manual' | 'auto'): Promise<BackupInfo> {
  ...
  // VACUUM INTO nie nadpisuje istniejących plików - nazwa zawiera znacznik czasu.
  db.prepare('VACUUM INTO ?').run(target);

  const stat = fs.statSync(target);
  pruneOldBackups();

  logger.info(`Utworzono kopię zapasową: ${fileName} (${stat.size} B)`);
  return {
    fileName,
...
function pruneOldBackups(): void {
  ...
  for (const stale of backups.slice(env.backupKeep)) {
    ...
    logger.info(`Usunięto przeterminowaną kopię zapasową: ${stale.fileName}`);
  }
}

/** Uruchamia cykliczne kopię zapasowe. Zwraca funkcje zatrzymujaca harmonogram. */
export function startBackupSchedule(db: Db): () => void {
  ...
  const timer = setInterval(() => {
    createBackup(db, 'auto').catch((err) => logger.error('Kopia zapasową nie powiodła się', err));
  }, intervalMs);
  timer.unref?.();

  logger.info(`Harmonogram kopii zapasowych: co ${env.backupIntervalHours} h, przechowywane ${env.backupKeep} plików.`);
